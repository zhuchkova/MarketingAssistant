...
The `cta_patterns.json` collection includes CTA examples and principles across `comment`, `dm_keyword`, `save`, `follow`, `download`, `share`, `book_visit`, `buy_order`, DM resource keyword CTAs, public replies, and no-guide fallbacks.

The `comment_automation_templates.json` collection includes flow templates with public comment replies, first message, qualification question, follow-up guidance, DM resource delivery, no-guide fallbacks, booking/order fallbacks, and manual ManyChat setup JSON.

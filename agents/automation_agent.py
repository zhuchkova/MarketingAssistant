...
class ManyChatSetup(BaseModel):
    ...
    resource_used: bool = Field(description="Whether a saved DM resource or custom URL was used")
    automation_resource_url: str = Field(description="DM resource or offer URL, or an empty string if none exists")
...
You are an Instagram automation strategist.

Create a reusable ManyChat-style Instagram comment-to-DM automation for this DM resource.

The automation may later be attached to many Instagram reels or carousels. If the post fields below are blank, ignore them and base the automation on the creator, audience, and selected DM resource.
...
- Suggested keyword: {automation_resource_keyword}
- Comment trigger mode: {automation_resource_trigger_type}
- Public comment reply: {automation_resource_public_comment_reply}
- Delivery message: {automation_resource_delivery_message}
- Second DM message: {automation_resource_second_dm_message}
- Opening DM button label: {automation_resource_opening_dm_button_label}
- Link button label: {automation_resource_link_button_label}
- Qualification question: {automation_resource_qualification_question}
...
Relevant comment-to-DM automation templates:
{manychat_knowledge}
...
- Make first_message specific to this DM resource, creator, and audience. Use the resource title, description, outcome, niche, or local context when available.
...
- If a DM resource URL exists, the first message must NOT include that URL; the URL belongs in the link step after the user clicks the opening button.
- If no DM resource URL exists, create a useful goal-based automation: booking details for book_visit, order details for buy_order, or a simple next-step conversation for comment/save/share/follow. Do not pretend a link exists.
...
- manychat_setup must be JSON-serializable and include:
  ...
  - flow_type
  - resource_used
  - automation_resource_url as a URL string or an empty string if no URL exists
  - setup_steps
  - api_supported_parts
...
def run_automation_agent(context: dict) -> dict:
    rag_knowledge = retrieve_automation_knowledge(context)
